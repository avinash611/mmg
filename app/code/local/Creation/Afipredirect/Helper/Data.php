...
class Creation_Afipredirect_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getVisitorsIpCountrySession() {
        ...
        // print_r($session);
        //die;
        //$session->unsetData($this->getCountryLookupService() . "_visitor_ip_country");       
    ...
    public function isSearchEngineBot() {
        ...
        foreach ($bots as $b) {
            ...
            if (stripos(Mage::helper('core/http')->getHttpUserAgent(), $b) !== false)
                return true;
        }
        return false;
    }
    ...
    public function getCountryCodeByIp($ip_address = false) {
        ...
        switch ($lookup_service) {
            case 'ip-api':
                $result = $this->ipapi_visitor_country($ip_address);
                break;
            case 'ipinfo':
                $result = $this->ipinfo_visitor_country($ip_address);
                break;
            case 'geoplugin':
    ...
    private function ipapi_visitor_country($ip_address = false) {
        ...
        $result = false;
        if (!empty($ip_address)) {
            $locaInfo = @unserialize(file_get_contents('http://ip-api.com/php/' . $ip_address));
            if ($locaInfo && $locaInfo['status'] == 'success') {
                if ($locaInfo['countryCode'] != null) {
                    $result = $locaInfo['countryCode'];
                }
            }
        }
        return $result;
    ...
    private function ipinfo_visitor_country($ip_address = false) {
        ...
        $result = false;
        if (!empty($ip_address)) {
            $location_api = 'http://ipinfo.io/' . $ip_address;
            $response = json_decode($this->request($location_api), true);
            if (!empty($response)) {
