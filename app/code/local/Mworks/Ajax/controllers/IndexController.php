...
class Mworks_Ajax_IndexController extends Mage_Checkout_CartController
{
	public function addAction()
	{
		...
		if($params['isAjax'] == 1){
			...
			try {
				if (isset($params['qty'])) {
					if(!preg_match('/^[0-9]\d*$/',$params['qty']) ) {
						$response['status'] = 'ERROR';
						$validNumberMsg = "Please enter a valid quantity.";
						$response['message'] = $validNumberMsg;
				...
				$cart_products = Mage::getSingleton('checkout/session')->getQuote()->getAllVisibleItems();
				foreach ($cart_products as $_products) {
					$cart_product_ids[$_products->getProductId()] = $_products->getQty();
				}
				
				if(sizeof($cart_product_ids) > 0) {
					if(array_key_exists($product->getId(), $cart_product_ids)) {
						$message = $this->__('<span>%s</span> <p>&nbsp; is already present in your shopping cart.</p>', Mage::helper('core')->htmlEscape($product->getName()));
						$response['status'] = 'SUCCESS';
						$response['message'] = $message;
						$response['qty'] = Mage::helper('checkout/cart')->getItemsCount();
						//New Code Here
					$this->loadLayout();
				...
				if (!$cart->getQuote()->getHasError()){
					$message = $this->__('<span>%s</span> <p>&nbsp; is added to your shopping cart.</p>', Mage::helper('core')->htmlEscape($product->getName()));
					$response['status'] = 'SUCCESS';
					$response['message'] = $message;
					$response['qty'] = Mage::helper('checkout/cart')->getItemsCount();
					 
					 /* Facebook event track starts here */
                    $productInfo['name'] = addslashes($product->getName());
                    $productInfo['sku'] = $product->getSku();

                    $categoryIds = $product->getCategoryIds();
                    $category = new Mage_Catalog_Model_Category();
                    $category->load(end($categoryIds));
                    $productInfo['category'] = addslashes($category->getName());

                    $productInfo['price'] = $product->getFinalPrice();
                    $response["productDetails"] = $productInfo;

                    /* Facebook event track ends here */
	...
	public function ajaxUpdateAction()
	{
        ...
        try {
            ...
            if (!$this->_getSession()->getNoCartRedirect(true)) {
                if (!$cart->getQuote()->getHasError()){
                    $message = $this->__('<span>%s</span> <p>&nbsp; is updated in your shopping cart.</p>', Mage::helper('core')->htmlEscape($item->getProduct()->getName()));
					$response['status'] = 'SUCCESS';
					$response['message'] = $message;
					$response['qty'] = Mage::helper('checkout/cart')->getItemsCount();
					//New Code Here
	...
	public function ajaxDeleteAction()
	{
		...
		if ($id) {
			try {  
				...
				//////////
				$cart_count = Mage::helper('checkout/cart')->getItemsCount();
				//////////
				$this->loadLayout(); 
				$sidebar_block = $this->getLayout()->getBlock('minicart_head');
				$sidebar = $sidebar_block->toHtml();
				...
				Mage::register('referrer_url', $this->_getRefererUrl());
				if ($cart_count > 0) {
					$message = $this->__('Product removed from your shopping cart.');
				} else {
					$message = $this->__('');
				}
				$response['status'] = 'SUCCESS';
						...
						try {
							...
							if (count($wishListItemCollection)) {
							    ...
							    foreach ($wishListItemCollection as $item) {
							        $wishlist_product = $item->getProduct();
							        $arrProductIds[] = $wishlist_product->getId();
							    }

							    if(in_array($productId, $arrProductIds)) {
									$message = $this->__('<span>%s</span> <p>&nbsp; is already present in your Wishlist.</p>', Mage::helper('core')->htmlEscape($product->getName()));
									$response['status'] = 'SUCCESS';
									$response['message'] = $message;
									$this->getResponse()->setHeader('Content-type', 'application/json');
        							$this->getResponse()->setBody( (string) $this->getRequest()->getParam('callback').Mage::helper('core')->jsonEncode($response));
                            ...
                            $productInfo['name'] = addslashes($product->getName());
                            $productInfo['sku'] = trim($product->getSku());
