...
class Mapmygenomie_Compare_Model_Optimage {
    ...
    public function getOptionCollection($attrID, $byOptions = false) {
        ...
        if ($optionCollection->getSize() > 0) {
            ...
            $imagePath = Mage::helper('mapmygenomie_compare/optimage')->getImageBaseDir();
...
//                }
    ...
    public function uploadAndSaveImages($observer) {
        ...
        if ($session->getAttributeData()) {
            ...
            $session->addError(Mage::helper('mapmygenomie_compare/optimage')->__('No attribute option image(s) was saved'));
            return $this;
        }
        ...
        $destinationFolder = Mage::helper('mapmygenomie_compare/optimage')->getImageBaseDir();
        $toInsert = array();
        $toDelete = array();
        ...
        if (count($toInsert)) {
            Mage::getResourceModel('mapmygenomie_compare/hashcode')->insertValues($toInsert);
        }
        if (count($toDelete)) {
            Mage::getResourceModel('mapmygenomie_compare/hashcode')->deleteValues($toDelete);
        }
        if ($deleted) {
            $session->addSuccess(Mage::helper('mapmygenomie_compare/optimage')->__('%s attribute option image(s) deleted', $deleted));
        }

        if ($success) {
            $session->addSuccess(Mage::helper('mapmygenomie_compare/optimage')->__('%s attribute option image(s) uploaded', $success));
        }
        if ($success || $deleted) {
            //clean cache
            Mage::helper('mapmygenomie_compare/optimage')->cleanCache();
        }
    }
    ...
    public function getHashCodes($optionIds, $asArray = true) {
        $collection = Mage::getModel('mapmygenomie_compare/hashcode')->getCollection()
