...
class SmsWeb_SMSNotifications_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getSettings() {
        ...
        // Get the  settings
        $settings['sms_enable'] = Mage::getStoreConfig('smsnotifications/sms_api_credentials/enable',Mage::app()->getStore()->getStoreId());
    ...
    public function sendSms($body, $recipients = array()) {
        ...
        $uri = $start . $api_url . "/api/v3/?format=json&method=sms&api_key=" . urlencode($settings['sms_auth_token']) . "&sender=" . urlencode($settings['sms_sender_name']) . "&to=" . urlencode(implode(',', $recipients)) . "&message=" . urlencode($body);

        $result = file_get_contents($uri);
        $rows = json_decode($result, true);

        file_put_contents('errorsms.txt', "$uri$rows[message]");
